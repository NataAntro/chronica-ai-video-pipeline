...
import { scenarioPromptVersion } from "../prompts/scenario.js";
import {
  describeArtifact,
  readManifest,
  resolveArtifactPath,
...
const safeRunIdPattern = /^[a-zA-Z0-9][a-zA-Z0-9_-]{0,63}$/;
const speechMaxCharacters = 120;

const fingerprint = (value: object): string =>
  createHash("sha256").update(JSON.stringify(value)).digest("hex");

export const validateRunId = (runId: string): string => {
  if (!safeRunIdPattern.test(runId)) {
    throw new Error(
      "runId должен содержать от 1 до 64 символов: латинские буквы, цифры, '_' или '-'",
    );
  }
  return runId;
};
...
export const runPipeline = async (
  ...
  const runId = validateRunId(dependencies.runId ?? randomUUID());
  ...
  const topic = dependencies.topic ?? "Еженедельный технологический дайджест";
  const stageFingerprints: Record<PipelineStage, string> = {
    scenario: fingerprint({
      topic,
      promptVersion: scenarioPromptVersion,
      llmProfile: dependencies.llmProfile ?? dependencies.mode,
      evaluationContract: "script-evaluation-v2",
    }),
    narration: fingerprint({
      ttsProfile: dependencies.ttsProfile ?? dependencies.mode,
      speechMaxCharacters,
      audioContract: "pcm-wav-v2",
    }),
    subtitles: fingerprint({ timingAlgorithm: "audio-duration-v2" }),
    render: fingerprint({ composition: "tech-digest-v1", fps: 24 }),
  };
  ...
  const canReuse = async (
    ...
    if (entry.inputFingerprint !== stageFingerprints[stage]) {
      reason = "input_fingerprint_changed";
    } else if (invalidatedStages.has(stage)) {
  ...
  const executeStage = async <Result>(
    ...
    try {
      ...
      entry.inputFingerprint = stageFingerprints[stage];
  ...
  if (await canReuse("scenario", ["script", "scenario-trace"])) {
    script = parseVideoScript(
      JSON.parse(await readFile(artifactPath("script"), "utf8")),
    );
  } else {
    removeArtifacts(["script", "scenario-trace"]);
    script = await executeStage("scenario", async () => {
      const generated = await generateScenarioWithTrace(
        dependencies.llm,
        topic,
      );
      const scriptPath = join(runDirectory, "script.json");
      const tracePath = join(runDirectory, "scenario-trace.json");
      await writeFile(
        scriptPath,
        `${JSON.stringify(generated.script, null, 2)}\n`,
        "utf8",
      );
      await writeFile(
        tracePath,
        `${JSON.stringify(generated.trace, null, 2)}\n`,
        "utf8",
      );
      await recordArtifact("script", scriptPath);
      await recordArtifact("scenario-trace", tracePath);
  ...
  if (await canReuse("narration", ["audio", "narration-metadata"])) {
    const storedChunks = parseAudioChunks(
      JSON.parse(await readFile(artifactPath("narration-metadata"), "utf8")),
    );
    narration = {
    ...
    narration = await executeStage("narration", async () => {
      const ttsChunks = chunkSpeech(
        buildSpeechBlocks(script),
        speechMaxCharacters,
      );
  ...
  if (await canReuse("subtitles", ["subtitles"])) {
    subtitles = parseSubtitleCues(
      JSON.parse(await readFile(artifactPath("subtitles"), "utf8")),
    );
  } else {
